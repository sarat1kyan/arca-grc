<script lang="ts">
	...
	const emphasisClasses = emphasis
		? 'border-l-4 border-l-blue-500 bg-gradient-to-r from-blue-25 to-white shadow-md'
...
{#snippet cardContent()}
	<div class="flex-1 flex flex-col justify-center">
		<div
			class="text-2xl font-bold text-gray-800 leading-none mb-1 group-hover:text-blue-800 transition-colors duration-200"
		>
			{formattedCount()}
		</div>
		<div
			class="text-xs font-medium text-gray-600 capitalize group-hover:text-blue-700 transition-colors duration-200"
		>
			{label}
		</div>
	</div>
