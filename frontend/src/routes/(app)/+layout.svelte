...
<div class="overflow-x-hidden">
	...
	>
		{#snippet headline()}
			<span
				class="text-2xl font-bold pb-1 bg-linear-to-r from-pink-500 to-blue-600 bg-clip-text text-transparent"
				id="page-title"
			>
			...
			{#if data?.user?.is_admin}
				<button
					...
					class="absolute top-7 right-9 p-2 rounded-full bg-blue-500 text-white text-xs shadow-lg
        ring-2 ring-blue-400 ring-offset-2 transition-all duration-300 hover:bg-blue-600
        hover:ring-blue-300 hover:ring-offset-blue-100 hover:shadow-blue-500/50
        focus:outline-hidden focus:ring-blue-500"
				>
					{m.quickStart()}
				</button>
			{/if}
	...
	<CommandPalette />
	<main
		class="min-h-screen p-8 bg-linear-to-br from-blue-100 to-slate-200 transition-all duration-300 {classesSidebarOpen(
			sidebarOpen
		)}"
	>
		{@render children?.()}
